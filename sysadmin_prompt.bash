...
function __sypro_setup()
{
  ...
  function sypro_pick_color() {
    ...
    local int="$(__sypro_word2int "$1")"
    local color_index=$((32 + $(( $int % 6)) ))
  ...
  if type -t md5sum >/dev/null 2>&1; then
    ...
    function __sypro_word2int() {
      ...
      # we truncate $1 to 7 characters (<32bits) to avoid integer overflows
      # on most platforms and bash versions.
      #
      # we get the start of string, not end, to be robust against various
      # md5sum and bash versions (negative indexes not always work).
      echo "0x${1:0:7}"
    }
    ...
    function __sypro_word2int() {
      ...
      # interested in the first field, which will be the new $1 after this.
      set $(echo -n "$1" | cksum )
      echo "$1"
    }
  ...
  if [ "$1" ] && [ "$1" = install ]; then
    # works with 'source'
    if [ "$BASH_SOURCE" ] && [ -r "$BASH_SOURCE" ]; then
      local sypro_src="$BASH_SOURCE"
    ...
    if ! grep -v '^[[:space:]]*#' ~/.bashrc \
      ...
      ~/.bashrc
    then
      echo "[ -r '$sypro_src' ] && source '$sypro_src'" >> ~/.bashrc
...
function sypro_prompt_command() {
  ...
  if type -t __git_ps1 >/dev/null 2>&1; then
    local git="${sypro_colors[git]}$(__git_ps1 '::%s')$r"
  fi

  if [ "$__sypro_virt" ]; then
    local isvirt="[${sypro_colors[virt]}$__sypro_virt$r]"
  fi
